...
// ⬇ keep your constants/types (or import enums from '@prisma/client' if you prefer)
import { CATEGORIES, ASSIGNEES, type BillCategory, type BillAssignee } from '@/lib/bills';
...
export default function AddBillForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      setSubmitting(true);
      await addBill({
        // ✅ only send what the API needs; server sets id/status/currency/source
        name: name.trim(),
        amount: amt,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border bg-white p-4 shadow-sm">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500">Add Bill</h3>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-sm text-gray-700">Amount (PHP)</label>
          <input
            inputMode="decimal"
            className="w-full rounded-md border px-3 py-2 text-sm"
            placeholder="0.00"
        ...
        <div>
          <label className="mb-1 block text-sm text-gray-700">Provider (optional)</label>
          <input
            className="w-full rounded-md border px-3 py-2 text-sm"
      ...
      <div>
        <label className="mb-1 block text-sm text-gray-700">Notes (optional)</label>
        <textarea
          className="w-full rounded-md border px-3 py-2 text-sm"
          rows={3}
